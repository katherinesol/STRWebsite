...
-- SELF-CHECK. This returns a row, so seeing a RESULT rather than
-- "Success. No rows returned" proves the whole file ran, not just the first
-- statement. Both columns must come back true.
select
  position('reference'        in pg_get_functiondef('create_invoice_full(jsonb)'::regprocedure)) > 0 as reference_present,
  position('method_last4'     in pg_get_functiondef('create_invoice_full(jsonb)'::regprocedure)) > 0 as account_present,
  position('unique_violation' in pg_get_functiondef('create_invoice_full(jsonb)'::regprocedure)) > 0 as guard_present;
