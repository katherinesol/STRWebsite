-- True all-or-nothing invoice EDIT. Sibling of create_invoice_full.
--
-- PostgREST issues one statement per request, so editing line items from the API
-- means a sequence of independent writes: delete three rows, update two, insert
-- one. If any step fails the invoice is left half-edited, with a total that
-- matches neither what you had nor what you meant. This runs in one transaction.
--
-- The payload carries the DESIRED FULL STATE of items and adjustments. Rows in
-- the database whose id is absent from the payload are deleted; rows with a
-- matching id are updated; rows with a new id are inserted.
--
-- CATEGORY SYNC. An invoice's category is duplicated onto every expense its
-- payments created. Changing it in one place and not the other is the failure
-- this app keeps hitting, so the sync happens here, inside the same transaction:
-- either both move or neither does. It syncs CATEGORY ONLY. Amounts are never
-- touched — an expense records money that actually left the account, and editing
-- an invoice does not change what was paid.
--
-- One invoice has MANY linked expenses (one per payment), so the sync updates
-- all of them, not one.
